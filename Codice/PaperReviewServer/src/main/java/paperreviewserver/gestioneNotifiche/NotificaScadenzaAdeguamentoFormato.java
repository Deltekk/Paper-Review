...
public class NotificaScadenzaAdeguamentoFormato implements Job {
    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        try(Connection connect = DBMSBoundary.getConnection()) {
            ...
            for(Map.Entry<Integer, String> entry : conferenze.entrySet()) {
                ...
                int rate = conferenzaDao.getRateAccettazione(idConferenza);

                // 4.2.1 Scadenza Seconda Sottomissione Articoli
                LocalDateTime scadenza = conferenzaDao.getScadenzaSottomissione3(idConferenza);
                ...
                if (giorniRimanenti <= giorniPreavviso && !oggi.isAfter(giornoScadenza)) {
                    ...
                    for (int i = 0; i < numeroPapersDaSelezionare; i++) {
                        ...
                        String testoNotifica = String.format(
                                "Attenzione mancano %d giorni alla scadenza della sottomissione dell'adeguamento del formato nella conferenza: %s",
                        ...
                        String subject = "Notifica di imminente scadenza sottomissione Adeguamento Formato";
                        String body = "<p> Salve " + nome + " " + cognome +
                                ", le notifichiamo che ancora non ha sottomesso l'adeguamento del formato nella conferenza " +
            ...
            ConsoleLogger.error("Errore durante il controllo notifiche Adeguamento Formato: " + e.getMessage());
        }
